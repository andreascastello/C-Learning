...
Matrice* lireDepuisFichierMatrice (FILE* fichier)
{
    ...
    if (fgetc(fichier) == '[')
    {
        ...
        //printf ("La matrice %s c est bien ouverte\n", nomFichier);
        //fprintf (stdout, "Positions courante : %i\n\n", ftell(fichier));
        ...
        Matrice* mat = creeMatrice (nom, y, x, 0);
        double temp;
        for (int i =0; i < y; i++)
        {
            for (int j =0; j < x; j++)
...
*/
/*
    FILE* fichier1 = fopen ("matrice.txt", "w");
    ecrireDansFichierMatrice (mat, fichier1);
*/
    FILE* fichier2 = fopen ("matrice.txt", "r");
    Matrice* mat1 = lireDepuisFichierMatrice(fichier2);
