...
int main(int argc, char** argv)
 ...
 SDL_Event event;
     ...
     SDL_Window* fenetre = SDL_CreateWindow ("Exemple 5" ,SDL_WINDOWPOS_UNDEFINED ,SDL_WINDOWPOS_UNDEFINED ,990 ,700 ,SDL_WINDOW_SHOWN);
  ...
  bool change;
    ...
    for (int i = 0; i <= numLab; i++)
    {
        ...
        for ( int y = 0 ; y < DIMY; y++ )
        {
            ...
            {
                afficherImage ( x*16, y*16, renderer , "img/sol.bmp");
            ...
            else
            {
                ...
                switch (labyrinthe[i][y][x])
                {
                    ...
                    case  'C' :
                        afficherImage ( x*16, y*16, renderer , "img/sol.bmp");
                        afficherImage ( x*16, y*16, renderer , "img/key.bmp");break;
                    ...
                    case  ',' :
                        switch (i)
                        {
                            case 0 :
                                afficherImage ( x*16, y*16, renderer , "img/sol.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/coin.bmp");break;
                            case 1 :
                                afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/coin2.bmp");break;
                            case 2 :
                                afficherImage ( x*16, y*16, renderer , "img/sol3.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/coin3.bmp");break;
                        }
                        break;
                    ...
                    case  '*' : switch (i)
                    {
                        case 0:
                        afficherImage ( x*16, y*16, renderer , "img/sol.bmp");
                        afficherImage ( x*16, y*16, renderer , "img/pic.bmp");break;
                        case 1:
                        afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");
                        afficherImage ( x*16, y*16, renderer , "img/pic.bmp");break;
                        case 2:
                    ...
                    case  '#' :
                        switch (i)
                        {
                            ...
                            case 2 : afficherImage ( x*16, y*16, renderer , "img/mur3.bmp");break;
                        }
                        break;
                    case  '.' :
                        switch (i)
                        {
                            ...
                            case 0 :
                                afficherImage ( x*16, y*16, renderer , "img/sol.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/point.bmp");break;
                            case 1 :
                                afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/point2.bmp");break;
                            case 2 :
                                afficherImage ( x*16, y*16, renderer , "img/sol3.bmp");
                                afficherImage ( x*16, y*16, renderer , "img/point3.bmp");break;
                        }
                        break;

                    case  'P' :
                        afficherImage ( x*16, y*16, renderer , "img/sol3.bmp");
                        afficherImage ( x*16, y*16, renderer , "img/sortie.bmp");break;
                    case  'E' : switch(i)
                    {
                        case 1:
                            afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");
                            afficherImage ( x*16, y*16, renderer , "img/ennemi.bmp");break;
                        case 2:
                            afficherImage ( x*16, y*16, renderer , "img/sol3.bmp");
                            afficherImage ( x*16, y*16, renderer , "img/ennemi.bmp");break;
                    }break;
                    ...
                    case  't' :
                        afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");
                        afficherImage ( x*16, y*16, renderer , "img/etoile.bmp");break;
                    case  ' ' :break;
                    case  '_' :
                        switch (i)
                            {
                                  case 0: afficherImage ( x*16, y*16, renderer , "img/sol.bmp");break;
                                  case 1: afficherImage ( x*16, y*16, renderer , "img/sol2.bmp");break;
                                  case 2: afficherImage ( x*16, y*16, renderer , "img/sol3.bmp");break;
                        ...
                        case SDLK_z : if ( labyrinthe[numLab][joueur.y-1][joueur.x   ] != '#' || labyrinthe[numLab][joueur.y-1][joueur.x   ] != 'o')
                                {
                                    while (labyrinthe[numLab][joueur.y-1][joueur.x] != '#' && labyrinthe[numLab][joueur.y-1][joueur.x   ] != 'o')
                                    {
                                        ...
                                        switch (numLab)
                                        {
                                            case 0 :afficherImage ( joueur.x*16, joueur.y*16, renderer , "img/sol.bmp");break;
                                            case 1 :afficherImage ( joueur.x*16, joueur.y*16, renderer , "img/sol2.bmp");break;
                                            case 2 :afficherImage ( joueur.x*16, joueur.y*16, renderer , "img/sol3.bmp");break;
                                        }
                                        joueur.y--;
                    ...
                    case 0 : if (labyrinthe[numLab][ennemies[e].y-1][ennemies[e].x] != '#')
                            {
                                ...
                                afficherImage ( ennemies[e].x*16, ennemies[e].y*16, renderer , "img/sol2.bmp");
                                ennemies[e].y--;
                                labyrinthe[numLab][ennemies[e].y][ennemies[e].x] = 'E';
                                afficherImage ( ennemies[e].x*16, ennemies[e].y*16, renderer , "img/ennemi.bmp");
                            }
                ...
                for(int x = 0 ; x < DIMX; x++)
                {
                    switch(labyrinthe[i][y][x])
                    {
                        case  'o' :
                                    if (change == FALSE)
                                    {
                                        afficherImage ( x*16, y*16, renderer , "img/piegeF.bmp");
                                    }
                                    else if (change == TRUE)
                                    {
                                        afficherImage ( x*16, y*16, renderer , "img/piegeO.bmp");
                                    }break;
                        case  'S' :afficherImage ( x*16, y*16, renderer , "img/porteF.bmp");break;
                        case 's' :afficherImage ( x*16, y*16, renderer , "img/porteO.bmp");break;
